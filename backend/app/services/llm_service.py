...
async def analyze_project(project_title: str) -> AnalyzeResponse:
    """Call OpenAI or Gemini and parse the structured component list."""
    data = None
    ...
    if data is None and settings.GEMINI_API_KEY and settings.GEMINI_API_KEY.strip():
        try:
            import httpx
            url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={settings.GEMINI_API_KEY}"
            payload = {
                "systemInstruction": {"parts": [{"text": SYSTEM_PROMPT}]},
                "contents": [{"parts": [{"text": f"Project: {project_title}"}]}],
    ...
    if data is None:
        raise ValueError("No API key configured for LLM service (OpenAI or Gemini), or API calls failed.")
