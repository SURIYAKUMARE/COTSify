...
import { CATALOG_DATA, searchCatalog, getCatalogForProject } from "@/lib/catalog-data";
...
function getRecentlyViewed(): CatalogProduct[] {
...
function addRecentlyViewed(product: CatalogProduct) {
  if (typeof window === "undefined") return;
  try {
...
function CatalogContent() {
  ...
  const applyAdminOverrides = (data: CatalogProduct[]) => {
    try {
      const overrides = JSON.parse(localStorage.getItem("admin_catalog_overrides") || "{}");
      return data.map(p => overrides[p.id] ? { ...p, ...overrides[p.id] } : p);
    } catch { return data; }
  };

  const fetchAll = () => {
    const data = applyAdminOverrides(CATALOG_DATA);
    setProducts(data);
    setFiltered(data);
  };

  const fetchForProject = (title: string) => {
    const names = title.split(/[,\s]+/).filter(Boolean);
    const matched = getCatalogForProject(names);
    const data = applyAdminOverrides(matched.length > 0 ? matched : CATALOG_DATA);
    setProducts(data);
    setFiltered(data);
  };
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    const results = searchCatalog(searchInput);
    setProducts(results);
    applyFilters(results, selectedCategory, priceRange, sortBy);
  };
  ...
  const handleViewProduct = (p: CatalogProduct) => {
    addRecentlyViewed(p);
    setRecentlyViewed(getRecentlyViewed());
    ...
    <div className="min-h-screen">
      ...
      <div className="relative overflow-hidden bg-gradient-to-b from-gray-900 to-transparent border-b border-gray-800 px-4 py-10">
        ...
        <div className="max-w-3xl mx-auto relative">
          ...
          {/* Why buy from catalog? */}
          <div className="flex gap-2 justify-center mt-4 flex-wrap">
            <span className="text-gray-500 text-xs self-center">Why buy from catalog?</span>
            {[
              { icon: <Tag className="w-3.5 h-3.5" />, label: "Best prices", color: "bg-green-950/60 text-green-400 border-green-800/50" },
              { icon: <BadgeCheck className="w-3.5 h-3.5" />, label: "Verified components", color: "bg-blue-950/60 text-blue-400 border-blue-800/50" },
              { icon: <Truck className="w-3.5 h-3.5" />, label: "Fast delivery", color: "bg-purple-950/60 text-purple-400 border-purple-800/50" },
            ].map((chip) => (
              <span key={chip.label} className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border font-medium ${chip.color}`}>
          ...
          {loading ? (
            ...
            Object.entries(grouped).map(([cat, items]) => items.length > 0 && (
              <div key={cat} className="mb-10">
                ...
                <ProductGrid items={items} viewMode={viewMode} compareList={compareList} wishlist={wishlist} onCompare={toggleCompare} onWishlist={toggleWishlist} onView={handleViewProduct} />
              </div>
            ))
          ) : (
            <ProductGrid items={filtered} viewMode={viewMode} compareList={compareList} wishlist={wishlist} onCompare={toggleCompare} onWishlist={toggleWishlist} onView={handleViewProduct} />
          )}
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 pb-10">
          <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-5 backdrop-blur-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-gray-500">({recentlyViewed.length} items)</span>
            </div>
              ...
              {recentlyViewed.map((p) => (
                <button key={p.id} onClick={() => handleViewProduct(p)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-cyan-400 text-sm font-bold">{p.price_inr ? `₹${p.price_inr.toLocaleString()}` : "POA"}</p>
                  </div>
                </button>
              ))}
            </div>
      ...
      {compareList.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-gray-950/95 backdrop-blur border-t border-gray-800 px-4 py-3 flex items-center gap-3 flex-wrap">
          ...
          {compareList.length >= 2 && (
            <Link href={`/compare?ids=${compareList.map(p => p.id).join(",")}`} className="ml-auto bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-semibold px-5 py-2 rounded-xl text-sm transition-all">
              Compare Now
            </Link>
          )}
        </div>
