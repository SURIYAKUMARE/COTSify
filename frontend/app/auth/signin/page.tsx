...
export default function SignInPage() {
  ...
  const handleGoogle = async () => {
    ...
    if (result.error) { setError(result.error); setSubmitting(false); }
    // On success, browser redirects to /auth/callback — no need to do anything
  };
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    if (!sb) { setError("Supabase not configured. Add your keys to .env.local"); return; }
    setSubmitting(true);
    const { error } = await sb.auth.signInWithOtp({ phone: normalized });
    if (error) { setError(error.message); setSubmitting(false); }
    else { setPhone(normalized); setOtpSent(true); setSubmitting(false); }
  };
              ...
              {mode === "phone" && otpSent && (
                <form onSubmit={handleVerifyOtp} className="flex flex-col gap-4">
                  <div className="text-center">
                    ...
                    <p className="text-gray-400 text-sm mt-1">Enter the 6-digit code</p>
                  </div>
